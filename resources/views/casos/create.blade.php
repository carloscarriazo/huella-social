<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nuevo Caso</h2>
    </x-slot>

    <div class="py-4 max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow p-6">
            <form method="POST" action="{{ route('casos.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        ...
                        <option value="familia" {{ old('tipo') === 'familia' ? 'selected' : '' }}>Familia</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea id="descripcion" name="descripcion" rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-teal-500 focus:border-teal-500">{{ old('descripcion') }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
                    <a href="{{ route('casos.index') }}" class="text-gray-600 hover:text-gray-800 text-sm">Cancelar</a>
                    <x-primary-button>Guardar Caso</x-primary-button>
                </div>
            </form>
        </div>
    </div>
